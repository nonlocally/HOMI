#!/usr/bin/env bash
# Homi↔homi link test: two isolated homis on one host,
...
T="$(mktemp -d /tmp/homi-link.XXXXXX)"
...
echo "== setup: two homis, symmetric links"
acomm homi start >/dev/null 2>&1 || bad "A start"
bcomm homi start >/dev/null 2>&1 || bad "B start"
bcomm homi claim remote-x >/dev/null 2>&1 || bad "B claim remote-x"
# A's outbound targets B's inbound-for-alpha; B's outbound targets A's inbound-for-beta.
if acomm homi link beta --sock "$B_STATE/homi/in/alpha.sock" >/dev/null 2>&1; then ok "A link beta"; else bad "A link beta"; fi
if bcomm homi link alpha --sock "$A_STATE/homi/in/beta.sock" >/dev/null 2>&1; then ok "B link alpha"; else bad "B link alpha"; fi
[ -S "$A_STATE/homi/in/beta.sock" ] && ok "A inbound-for-beta bound" || bad "A inbound-for-beta bound"
[ -S "$B_STATE/homi/in/alpha.sock" ] && ok "B inbound-for-alpha bound" || bad "B inbound-for-alpha bound"

echo "== cross-homi delivery + ack"
acomm homi send remote-x@beta "hello across" --from tester >/dev/null 2>&1 || bad "A send remote-x@beta"
BX="$B_STATE/homi/mail/remote-x/inbox.jsonl"
...
sleep 1
qn="$(ls "$A_STATE/homi/out/beta" 2>/dev/null | wc -l | tr -d ' ')"
...
wait_for 6 "B grew proxy socket for tester" test -S "$B_SOCKS/homi-tester.sock"
tside=""
...
AT="$A_STATE/homi/mail/tester/inbox.jsonl"
...
bcomm homi stop >/dev/null 2>&1; sleep 1
acomm homi send remote-x@beta "while down" --from tester >/dev/null 2>&1
sleep 2
qn="$(ls "$A_STATE/homi/out/beta" 2>/dev/null | wc -l | tr -d ' ')"
if [ "$qn" = "1" ]; then ok "queued while beta down"; else bad "queued while beta down (got $qn)"; fi
bcomm homi start >/dev/null 2>&1
...
echo "== dedup by msg_id at the receiving homi"
python3 - "$B_STATE/homi/in/alpha.sock" <<'PY'
...
case "$cmd" in
  "ssh -N -o BatchMode=yes -o ConnectTimeout=8 -o ExitOnForwardFailure=yes -o ServerAliveInterval=15 -o ServerAliveCountMax=3 -o StreamLocalBindMask=0177 -o StreamLocalBindUnlink=yes -L $A_STATE/homi/links/gamma.sock:<REMOTE_HOME>/.local/state/communicate/homi/in/alpha.sock fake@nowhere.invalid")
    ok "golden ssh dial command";;
  *) bad "golden ssh dial command (got: $cmd)";;
esac
acomm homi link gamma --addr fake@nowhere.invalid >/dev/null 2>&1 || bad "link gamma (addr)"
acomm homi send remote-y@gamma "never arrives" --from tester >/dev/null 2>&1
sleep 3
qn="$(ls "$A_STATE/homi/out/gamma" 2>/dev/null | wc -l | tr -d ' ')"
...
acomm homi unlink gamma >/dev/null 2>&1 && ok "unlink gamma" || bad "unlink gamma"
