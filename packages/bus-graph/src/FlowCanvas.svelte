<script>
  ...
  const nodeTypes = {agent: AgentNode, device: DeviceNode};
  const flow = useSvelteFlow();
  export function fit() { return flow.fitView({padding: 0.2, maxZoom: 1, duration: 180}); }
</script>
<SvelteFlow bind:nodes bind:edges {nodeTypes} fitView fitViewOptions={{padding: 0.2, maxZoom: 1}} colorMode="dark" minZoom={0.18} maxZoom={1.8} nodesConnectable={false} edgesReconnectable={false} edgesFocusable={false} deleteKey={null} multiSelectionKey={null} selectionKey={null} selectNodesOnDrag={false} preventScrolling={true} panOnScroll={true} zoomOnScroll={false} zoomOnPinch={true} zoomActivationKey={['Control', 'Meta']} panOnDrag={[0, 1, 2]} onnodedragstart={ondragstart} onnodedragstop={ondragstop} onnodeclick={({node}) => onselect(node.id)} onselectionchange={({nodes: selection}) => onselect(selection.find(node => node.type === 'agent')?.id || null)} onpaneclick={() => onselect(null)}>
