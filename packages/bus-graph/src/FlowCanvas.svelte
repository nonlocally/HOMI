<script>
  ...
  let { nodes = $bindable([]), edges = $bindable([]), onselect } = $props();
...
<SvelteFlow bind:nodes bind:edges {nodeTypes} fitView fitViewOptions={{padding: 0.2, maxZoom: 1}} colorMode="dark" minZoom={0.18} maxZoom={1.8} nodesConnectable={false} edgesReconnectable={false} edgesFocusable={false} deleteKey={null} multiSelectionKey={null} selectionKey={null} selectNodesOnDrag={false} preventScrolling={false} onnodeclick={({node}) => onselect(node.id)} onselectionchange={({nodes: selection}) => onselect(selection.find(node => node.type === 'agent')?.id || null)} onpaneclick={() => onselect(null)}>
