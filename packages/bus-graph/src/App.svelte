<script>
  ...
  import { snapshotModel, layoutNodes, decorate, agentDetails, agentNodeId, owner, deviceName } from './model.mjs';
  ...
  onDestroy(() => { disposed = true; cancelFit(); });
...
<section class="cg-shell" class:cg-expanded={expanded} aria-label="Agent connections graph">
  <header class="cg-toolbar">
    ...
    <div class="cg-actions"><span class="cg-visual-label">Visual layout</span><button type="button" onclick={() => canvas?.fit()} disabled={!model.agents.length}>Fit</button><button type="button" onclick={resetLayout} disabled={!model.agents.length}>Reset layout</button><button type="button" bind:this={expandButton} onclick={toggleExpanded} aria-expanded={expanded} aria-label={expanded ? 'Collapse graph canvas' : 'Expand graph canvas'}>{expanded ? 'Collapse' : 'Expand'}</button></div>
  </header>
  <div class="cg-workspace" class:cg-with-inspector={details !== null}>
    <div class="cg-canvas" aria-label="Draggable graph of agents grouped by device">
      {#key epoch}
        <SvelteFlowProvider><FlowCanvas bind:this={canvas} bind:nodes bind:edges onselect={select} /></SvelteFlowProvider>
      {/key}
    ...
    {#if details}
      <aside class="cg-inspector" aria-label="Selected agent details">
  ...
  <footer class="cg-footer"><span>Drag agents to arrange · Select to inspect · Scroll to zoom</span><span>{messageCount} retained messages · Positions change this view only</span></footer>
</section>
