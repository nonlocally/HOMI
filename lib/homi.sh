...
homi_cmd() {
  ...
  case "$sub" in
    ...
    stop|status|agents|wait|claim|release|send|ask|reply|group|notify|seat|spawn|fan|consult|link|unlink|grant|ungrant|grants|card|federate) python3 "$HOMI_PY" call "$sub" "$@";;
    inbox)     homi_inbox "$@";;
