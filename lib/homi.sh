...
homi_cmd() {
  ...
  case "$sub" in
    ...
    stop|status|agents|wait|claim|release|describe|send|ask|reply|group|notify|seat|spawn|restart|fan|consult|link|unlink|grant|ungrant|grants|card|user|init|pair|federate|statepath|premove|depart|arrive|move) python3 "$HOMI_PY" call "$sub" "$@";;
    inbox)     homi_inbox "$@";;
    install)   homi_install "$@";;
    uninstall) homi_uninstall "$@";;
    adopt)     homi_adopt "$@";;
    retitle)   python3 "$HOMI_PY" retitle "$@";;
    *) die "usage: communicate homi {start|stop|status|agents|init|user|claim [--boxed]|release|describe|send|ask|reply|wait|group|notify|inbox|seat|spawn|restart|fan|consult|move|link|unlink|grant|ungrant|grants|card|pair|federate|install|uninstall|adopt|retitle} ...";;
  esac
}
