...
pm_cmd() {
  ...
  case "$sub" in
    ...
    adopt)     pm_adopt "$@";;
    retitle)   python3 "$PM_PY" retitle "$@";;
    *) die "usage: communicate pm {start|stop|status [--json]|claim|release|send|inbox|link|unlink|install|uninstall|adopt|retitle} ...";;
  esac
}
...
pm_adopt() {
  ...
  [ -n "$name" ] || die "usage: communicate pm adopt <name> --pane <paneid>"
  [ -n "$pane" ] || die "adopt needs --pane <paneid> (live rename rides pane send); for a dormant session use: communicate pm retitle <uuid> $name"
